namespace EmergencyServicesBot
{
    ...
    public class IVRBot : IDisposable, ICallingBot
    {
        ...
        private Task OnIncomingCallReceived(IncomingCallEvent incomingCallEvent)
        {
            ...
            //var displayName = incomingCallEvent.IncomingCall.Participants.FirstOrDefault().DisplayName;
            var from = incomingCallEvent.IncomingCall.Participants.First(x => !x.Originator);
            var record = new Record
            {
                OperationId = Guid.NewGuid().ToString(),
                PlayPrompt = new PlayPrompt { OperationId = Guid.NewGuid().ToString(), Prompts = new List<Prompt> { new Prompt { Value = "Please leave a message" + from.DisplayName } } },
                RecordingFormat = RecordingFormat.Wav
            };
